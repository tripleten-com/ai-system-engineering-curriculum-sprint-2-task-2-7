...
# Task 2.7 assesses one configuration change and one regression test. Keep
# this in sync with submission_validation.ALLOWED_PATHS and ALLOWED_PREFIXES.
# `submission.yaml` is written by hand. The two retained benchmark reports are
# written by `poe benchmark-baseline` and `poe benchmark-experiment` and are
# required run artifacts, so they are permitted in the diff without being
# student-editable: the report contract and the assessed checks reject one that
# does not describe this template, these fixtures, and that configuration.
SUBMISSION_DIFF_ALLOWLIST = frozenset(
    {"submission.yaml", ".benchmark/baseline.json", ".benchmark/experiment.json"}
)
SUBMISSION_DIFF_PREFIXES = ("config/student/", "tests/student/")
HEADER_FIELDS = (
    "File:",
    "Component:",
    "Purpose:",
...
@pytest.mark.parametrize("fixture", ["infra/corpus/README.md", "infra/judge/README.md"])
...
def test_submission_change_stays_within_the_permitted_diff() -> None:
    """Catch a submitted change that reaches outside Task 2.7's implementation boundary."""
    changed = {
